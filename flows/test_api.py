...
class GraphFilesTest(unittest.TestCase):
    def test_default_is_case_dag(self):
        ...
        self.assertEqual(graph["metadata"]["lab"]["wall"], "v11-hardened")
        self.assertEqual(graph["metadata"]["lab"]["hunted"], "v10-hardened")
        self.assertEqual(graph["metadata"]["lab"].get("fill"), "unavailable")
        self.assertEqual(graph["metadata"]["lab"].get("fill_wall"), "v10-hardened")
        self.assertEqual(graph["metadata"]["lab"].get("next_hunt"), "v11-hardened")
        ...
        self.assertEqual(graph["metadata"]["lab"].get("last_live_wall"), "v9-hardened")
        ...
        self.assertEqual(graph["metadata"]["lab"].get("fill_score_get"), "unavailable")
        self.assertEqual(graph["metadata"]["lab"].get("fill_deny_403"), "unavailable")
        self.assertIn("unavailable", graph["metadata"]["lab"].get("fill_reason") or "")
        self.assertEqual(graph["version"], "1.11.0")
        report_to_harden = [
    ...
    def test_studio_shows_score_and_wall_pills(self):
        ...
        self.assertEqual(template["metadata"]["lab"]["wall"], "v11-hardened")
        self.assertEqual(template["metadata"]["lab"]["hunted"], "v10-hardened")
        self.assertEqual(template["metadata"]["lab"].get("fill"), "unavailable")
        self.assertEqual(template["metadata"]["lab"].get("next_hunt"), "v11-hardened")
        self.assertTrue(template["metadata"]["lab"].get("applies") is True)
        self.assertEqual(template["metadata"]["lab"].get("last_live_wall"), "v9-hardened")
        self.assertEqual(template["metadata"]["lab"].get("last_live_score_get"), 200)
        self.assertEqual(template["metadata"]["lab"].get("fill_score_get"), "unavailable")
...
class ApiTest(unittest.TestCase):
    ...
    def test_empty_post_upserts_template(self):
        ...
        self.assertEqual(listed["graphs"][0].get("wall"), "v11-hardened")
        self.assertEqual(listed["graphs"][0].get("hunted"), "v10-hardened")
        self.assertEqual(listed["graphs"][0].get("last_score"), "0/116")
        self.assertEqual(listed["graphs"][0].get("fill"), "unavailable")
        self.assertEqual(listed["graphs"][0].get("fill_wall"), "v10-hardened")
        self.assertEqual(listed["graphs"][0].get("next_hunt"), "v11-hardened")
        ...
        self.assertEqual(listed["graphs"][0].get("last_live_wall"), "v9-hardened")
        ...
        self.assertEqual(listed["graphs"][0].get("fill_score_get"), "unavailable")
        self.assertEqual(listed["graphs"][0].get("fill_deny_403"), "unavailable")
    ...
    def test_score_pill_uses_last_score_from_versions(self):
        ...
        versions.write_text(
            '{"wall":"v11-hardened","hunted":"v10-hardened","last_score":"0/116","n":0,"N":116,"fill":"unavailable","fill_wall":"v10-hardened","fill_reason":"Fill unavailable this hunt on v10-hardened: GET /api/Challenges/ connection refused (docker not installed). Honest score 0/116. Last live Fill 0/116 on v9-hardened APPLIES.","docker_disabled_env":18,"coding_challenges":"separate /snippets — not mixed into n/N","applies":true,"applies_reason":"Last live Fill APPLIES on v9; this hunt Fill unavailable on v10","applies_erofs":"gone","applies_readonly_rootfs":false,"applies_tmpfs":"/tmp only","data_static_visible":true,"data_static_challenges_yml":1593,"data_static_security_questions_yml":29,"last_live_fill":"live","last_live_score":"0/116","last_live_wall":"v9-hardened","last_live_score_get":200,"last_live_deny_403":["/","/login","/api"],"score_path":"GET = /api/Challenges/","bind":"127.0.0.1:3000","fill_score_get":"unavailable","fill_deny_403":"unavailable"}\n'
        )
        status, data = get(f"{self.base}/api/case/score?program=juice-shop")
        self.assertEqual(status, 200)
        self.assertEqual(data["score"], "0/116")
        self.assertEqual(data.get("last_score"), "0/116")
        self.assertEqual(data.get("wall"), "v11-hardened")
        self.assertEqual(data.get("hunted"), "v10-hardened")
        self.assertEqual(data.get("fill"), "unavailable")
        self.assertEqual(data.get("fill_wall"), "v10-hardened")
        self.assertEqual(data.get("next_hunt"), "v11-hardened")
        ...
        self.assertEqual(data.get("last_live_wall"), "v9-hardened")
        ...
        self.assertEqual(data.get("fill_score_get"), "unavailable")
        self.assertEqual(data.get("fill_deny_403"), "unavailable")
