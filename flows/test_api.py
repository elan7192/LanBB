...
class GraphFilesTest(unittest.TestCase):
    def test_default_is_case_dag(self):
        ...
        self.assertEqual(graph["metadata"]["lab"]["wall"], "v17-hardened")
        self.assertEqual(graph["metadata"]["lab"]["hunted"], "v16-hardened")
        self.assertEqual(graph["metadata"]["lab"].get("fill"), "live")
        self.assertEqual(graph["metadata"]["lab"].get("fill_wall"), "v16-hardened")
        self.assertEqual(graph["metadata"]["lab"].get("next_hunt"), "v17-hardened")
        ...
        self.assertEqual(graph["metadata"]["lab"].get("last_live_wall"), "v16-hardened")
        ...
        self.assertEqual(graph["version"], "1.16.0")
        report_to_harden = [
    ...
    def test_studio_shows_score_and_wall_pills(self):
        ...
        self.assertIn("Edge nginx worker_processes is 1 (source, OOM=false); Fill on v16 held it; do not use auto", html)
        ...
        self.assertEqual(template["metadata"]["lab"]["wall"], "v17-hardened")
        self.assertEqual(template["metadata"]["lab"]["hunted"], "v16-hardened")
        self.assertEqual(template["metadata"]["lab"].get("fill"), "live")
        self.assertEqual(template["metadata"]["lab"].get("next_hunt"), "v17-hardened")
        self.assertTrue(template["metadata"]["lab"].get("applies") is True)
        self.assertEqual(template["metadata"]["lab"].get("last_live_wall"), "v16-hardened")
...
class ApiTest(unittest.TestCase):
    ...
    def test_empty_post_upserts_template(self):
        ...
        self.assertEqual(listed["graphs"][0].get("wall"), "v17-hardened")
        self.assertEqual(listed["graphs"][0].get("hunted"), "v16-hardened")
        self.assertEqual(listed["graphs"][0].get("last_score"), "0/116")
        self.assertEqual(listed["graphs"][0].get("fill"), "live")
        self.assertEqual(listed["graphs"][0].get("fill_wall"), "v16-hardened")
        self.assertEqual(listed["graphs"][0].get("next_hunt"), "v17-hardened")
        ...
        self.assertEqual(listed["graphs"][0].get("last_live_wall"), "v16-hardened")
    ...
    def test_score_pill_uses_last_score_from_versions(self):
        ...
        versions.write_text(
            '{"wall":"v17-hardened","hunted":"v16-hardened","last_score":"0/116","n":0,"N":116,"fill":"live","fill_wall":"v16-hardened","fill_reason":"Fill live 0/116 from GET /api/Challenges/ on v16-hardened (HTTP 200). Wall APPLIES: default-deny 403 on /, /login, /api.","docker_disabled_env":18,"coding_challenges":"separate /snippets — not mixed into n/N","applies":true,"applies_reason":"v16 Fill APPLIES: GET 200, default-deny 403 on / /login /api","applies_erofs":"gone","applies_readonly_rootfs":false,"applies_tmpfs":"/tmp only","data_static_visible":true,"data_static_challenges_yml":1593,"data_static_security_questions_yml":29,"last_live_fill":"live","last_live_score":"0/116","last_live_wall":"v16-hardened","last_live_score_get":200,"last_live_deny_403":["/","/login","/api"],"score_path":"GET = /api/Challenges/","bind":"127.0.0.1:3000","fill_score_get":200,"fill_deny_403":["/","/login","/api"],"edge_floor_mem":"6m","edge_floor_pids":6,"edge_floor_reason":"Fill on v11: compose mem 4m/pids 4 FAILED. Fill on v12-v16: mem 6m/pids 6 held. Live floor mem 6m pids 6.","worker_processes":1,"worker_processes_oom":false,"worker_processes_source":true,"worker_processes_fill_patch":false,"worker_processes_reason":"Fill on v16: worker_processes 1 is source (no Fill patch). OOM=false. v17 keeps worker_processes 1."}\n'
        )
        status, data = get(f"{self.base}/api/case/score?program=juice-shop")
        self.assertEqual(status, 200)
        self.assertEqual(data["score"], "0/116")
        self.assertEqual(data.get("last_score"), "0/116")
        self.assertEqual(data.get("wall"), "v17-hardened")
        self.assertEqual(data.get("hunted"), "v16-hardened")
        self.assertEqual(data.get("fill"), "live")
        self.assertEqual(data.get("fill_wall"), "v16-hardened")
        self.assertEqual(data.get("next_hunt"), "v17-hardened")
        ...
        self.assertEqual(data.get("last_live_wall"), "v16-hardened")
