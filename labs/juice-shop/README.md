...
| `overlays/v15-hardened/` | Previous wall: working harden plus leftover remote-user/oauth-proxy/tracing/cloud-auth headers closed on the score path, leftover web3-walletExploitAddress/2FA-SPA/ftp-quarantine/solve-server-side/coupon HTTP routes closed, `worker_processes 1` (source, OOM=false on Fill), edge floor mem>=6m pids>=6 held |
