...
This overlay is a **working harden** that actually applies. Fill on v6 confirmed EROFS_GONE, ReadonlyRootfs=false, tmpfs=/tmp only, data/static visible (`challenges.yml` 1593, `securityQuestions.yml` 29). Loop 8 hunted v7 with no live GET `/api/Challenges/` (docker not installed); last live fill remains 0/116 on v6. v8 keeps those apply constraints. v5 juice `read_only` EROFS-skipped `.well-known/csaf` and `tmpfs /juice-shop/data` hid `data/static`. v8 does not copy those skipped locks.
