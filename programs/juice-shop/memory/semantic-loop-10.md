...
Loop 10 hunted the v9-hardened wall, not v8 and not stock. Fill on the box after that overlay scored live 0/116 (GET /api/Challenges/ HTTP 200). Wall APPLIES: default-deny 403 on /, /login, /api. Do not rediscover. CASE tools did not auto-pwn.
...
UX: Flow Studio names last live fill beside this hunt's fill (now live 0/116 on v9), plus score path, bind, GET 200 / 403 default-deny on catalog, inspector, and score API. Report draft carries the same Fill evidence so 0/N is not mistaken for a missed wall.
