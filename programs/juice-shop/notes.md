...
Loop 6 hunted `v5-hardened`. Fill **unavailable** (GET `/api/Challenges/` connection refused; docker not installed). Honest score **0/116**. Report path still wrote. Wall is now `v6-hardened` (login closed, GET/HEAD only, SPA/static leak closed, nginx `limit_req` burst>=1).
