...
Loop 8 hunted `v7-hardened`. Fill **unavailable** (GET `/api/Challenges/` connection refused; docker not installed). Honest score **0/116**. Last live fill remains **0/116** on `v6-hardened` APPLIES (EROFS_GONE, ReadonlyRootfs=false, tmpfs=/tmp only, data/static visible). Do not invent n. Report path still wrote. Wall is now `v8-hardened` (working harden with the same apply constraints; exact GET `/api/Challenges/`; localhost bind; leftover SPA/Web3/payment closed; nginx `limit_req` burst>=1).
